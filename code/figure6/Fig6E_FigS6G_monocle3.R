...
ggsave("./endothelial_monocle3.pdf",width = 10,height = 8)

Track_genes <- graph_test(cds,neighbor_graph = "principal_graph",cores=10)
library(tidyverse)
library(patchwork)
Track_genes_sig <- Track_genes%>% top_n(n=16,morans_I) %>%
  pull(gene_short_name) %>% as.character()
#Track_genes_sig <- Track_genes_sig[c(4:10)]
plot_genes_in_pseudotime(cds[Track_genes_sig,],min_expr = 0.5,ncol = 2)
ggsave("./monocle3_pseudotime.pdf",width = 12,height = 8)
plot_cells(cds,genes = Track_genes_sig,show_trajectory_graph = F,label_cell_groups = F,label_leaves = F)
ggsave("./monocle3_featureplot.pdf",width = 12,height = 8)
